...
USB_DEVICE_TASK_APP_DATA usb_device_task_appData;
...
static void APP_USBDeviceHIDEventHandler(USB_DEVICE_HID_INDEX hidInstance,
                                         ...
                                         uintptr_t            userData)
{
    USB_DEVICE_TASK_APP_DATA * usb_device_task_appData = (USB_DEVICE_TASK_APP_DATA *)userData;
    ...
    switch(event)
    {
        case USB_DEVICE_HID_EVENT_REPORT_SENT:
            ...
            usb_device_task_appData->isMouseReportSendBusy = false;
            break;
        ...
        case USB_DEVICE_HID_EVENT_SET_IDLE:
           ...
           USB_DEVICE_ControlStatus(usb_device_task_appData->handleUsbDevice, USB_DEVICE_CONTROL_STATUS_OK);

            /* save Idle rate received from Host */
            usb_device_task_appData->idleRate = ((USB_DEVICE_HID_EVENT_DATA_SET_IDLE*)eventData)->duration;
            break;

        case USB_DEVICE_HID_EVENT_GET_IDLE:

            /* Host is requesting for Idle rate. Now send the Idle rate */
            USB_DEVICE_ControlSend(usb_device_task_appData->handleUsbDevice, &(usb_device_task_appData->idleRate),1);
        ...
        case USB_DEVICE_HID_EVENT_SET_PROTOCOL:
            ...
            usb_device_task_appData->activeProtocol = *(USB_HID_PROTOCOL_CODE *)eventData;
        ...
        case  USB_DEVICE_HID_EVENT_GET_PROTOCOL:
            ...
            /* Host is requesting for Current Protocol. Now send the Idle rate */
             USB_DEVICE_ControlSend(usb_device_task_appData->handleUsbDevice, &(usb_device_task_appData->activeProtocol), 1);
...
static void APP_USBDeviceEventHandler(USB_DEVICE_EVENT event, void * eventData, uintptr_t context)
{
    ...
    switch(event)
    {
        case USB_DEVICE_EVENT_SOF:
            ...
            usb_device_task_appData.setIdleTimer++;
        ...
        case USB_DEVICE_EVENT_DECONFIGURED:
            ...
            usb_device_task_appData.usbDeviceIsConfigured = false;
        ...
        case USB_DEVICE_EVENT_CONFIGURED:
            ...
            if(configurationValue->configurationValue == 1)
            {
                ...
                USB_DEVICE_HID_EventHandlerSet(USB_DEVICE_HID_INDEX_0,
                        APP_USBDeviceHIDEventHandler, (uintptr_t)&usb_device_task_appData);
                
                usb_device_task_appData.usbDeviceIsConfigured = true;
                
                usb_device_task_appData.isMouseReportSendBusy = false;
                usb_device_task_appData.emulateMouse          = true;
            }
            break;
        ...
        case USB_DEVICE_EVENT_POWER_DETECTED:
            ...
            USB_DEVICE_Attach(usb_device_task_appData.handleUsbDevice);
        ...
        case USB_DEVICE_EVENT_POWER_REMOVED:
            ...
            USB_DEVICE_Detach(usb_device_task_appData.handleUsbDevice);
...
static void USB_Task (void)
{
    ...
    if(usb_device_task_appData.usbDeviceIsConfigured)
    {
        if(usb_device_task_appData.emulateMouse)
        {
            sent_dont_move = false;

            if(movement_length > 50)
            {
                usb_device_task_appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
                usb_device_task_appData.mouseButton[1] = MOUSE_BUTTON_STATE_RELEASED;
                usb_device_task_appData.xCoordinate =(int8_t)dir_table[vector & 0x07] ;
                usb_device_task_appData.yCoordinate =(int8_t)dir_table[(vector+2) & 0x07];
                vector ++;
                movement_length = 0;
            }
        }
        ...
        { 
            usb_device_task_appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
            usb_device_task_appData.mouseButton[1] = MOUSE_BUTTON_STATE_RELEASED;
            usb_device_task_appData.xCoordinate = 0;
            usb_device_task_appData.yCoordinate = 0;
        }

        if(!usb_device_task_appData.isMouseReportSendBusy)
        {
            if( ((sent_dont_move == false) && (!usb_device_task_appData.emulateMouse)) 
                || (usb_device_task_appData.emulateMouse))
            {

                /* This means we can send the mouse report. The
                   isMouseReportBusy flag is updated in the HID Event Handler. */
                usb_device_task_appData.isMouseReportSendBusy = true;

                /* Create the mouse report */
                MOUSE_ReportCreate(usb_device_task_appData.xCoordinate, 
                                   usb_device_task_appData.yCoordinate,
                                   usb_device_task_appData.mouseButton, 
                                   &mouseReport);
                          ...
                          (size_t)sizeof(mouseReport)) == 0)
                {
                    ...
                    if((usb_device_task_appData.xCoordinate == 0) && (usb_device_task_appData.yCoordinate == 0))
                    {
                        /* If the coordinate positions are 0, that means there
                         * is no relative change */
                        if(usb_device_task_appData.idleRate == 0)
                        {
                            usb_device_task_appData.isMouseReportSendBusy = false;
                        }
                        else
                        {
                             ...
                             */
                            if(usb_device_task_appData.setIdleTimer * APP_USB_CONVERT_TO_MILLISECOND
                                    >= usb_device_task_appData.idleRate * 4)
                            {
                ...
                if(usb_device_task_appData.isMouseReportSendBusy == true)
                {
                    ...
                    USB_DEVICE_HID_ReportSend(USB_DEVICE_INDEX_0,
                        &usb_device_task_appData.reportTransferHandle, (uint8_t*)&mouseReport,
                        sizeof(MOUSE_REPORT));
                    usb_device_task_appData.setIdleTimer = 0;
                }
                movement_length ++;
                sent_dont_move = true;
            }
...
/*******************************************************************************
  Function:
    void USB_DEVICE_TASK_APP_Initialize ( void )

  Remarks:
    See prototype in usb_device_task_app.h.
 */

void USB_DEVICE_TASK_APP_Initialize ( void )
{
    /* Place the App state machine in its initial state. */
    usb_device_task_appData.state = USB_DEVICE_TASK_APP_STATE_INIT;
    ...
    { 
        ...
        usb_device_task_appData.handleUsbDevice       = USB_DEVICE_HANDLE_INVALID;
        usb_device_task_appData.usbDeviceIsConfigured = false;
        usb_device_task_appData.emulateMouse          = true;
        usb_device_task_appData.isMouseReportSendBusy = false;
        usb_device_task_appData.xCoordinate           = 0;
        usb_device_task_appData.yCoordinate           = 0;
        usb_device_task_appData.idleRate              = 0;
        usb_device_task_appData.setIdleTimer          = 0;
        
        for (i=0; i<MOUSE_BUTTON_NUMBERS; i++)
        {
            usb_device_task_appData.mouseButton[i] = MOUSE_BUTTON_NUMBERS;
        }
     }
...
/******************************************************************************
  Function:
    void USB_DEVICE_TASK_APP_Tasks ( void )
...
void USB_DEVICE_TASK_APP_Tasks ( void )
{
    ...
    switch ( usb_device_task_appData.state )
    {
        /* Application's initial state. */
        case USB_DEVICE_TASK_APP_STATE_INIT:
        {
            bool appInitialized = true;
       

            /* Open the device layer */
            if (usb_device_task_appData.handleUsbDevice == USB_DEVICE_HANDLE_INVALID)
            {
                usb_device_task_appData.handleUsbDevice = USB_DEVICE_Open( USB_DEVICE_INDEX_0,
                                               DRV_IO_INTENT_READWRITE );
                appInitialized &= ( USB_DEVICE_HANDLE_INVALID != usb_device_task_appData.handleUsbDevice );
            }
            ...
            if (appInitialized)
            {
                ...
                USB_DEVICE_EventHandlerSet(usb_device_task_appData.handleUsbDevice,
                                           APP_USBDeviceEventHandler, 0);
            
                usb_device_task_appData.state = USB_DEVICE_TASK_APP_STATE_SERVICE_TASKS;
            }
            break;
        }

        case USB_DEVICE_TASK_APP_STATE_SERVICE_TASKS:
        {
            USB_Task();
